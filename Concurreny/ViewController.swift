...
class ViewController: UIViewController {
    ...
    func searchTerm(searchText: String) {
        countries = Country.getCountryData(data: data).filter({ ($0.name.lowercased().contains(searchText.lowercased()))})
    }
...
extension ViewController: UISearchBarDelegate {
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        searchTerm(searchText: searchBarText)
